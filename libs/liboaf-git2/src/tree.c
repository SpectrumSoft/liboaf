...
{
	if (path)
		giterr_set(GITERR_TREE, QT_TRANSLATE_NOOP("libgit2", "%s - %s"), str, path);
	else
		giterr_set(GITERR_TREE, QT_TRANSLATE_NOOP("libgit2", "%s"), str);
	return -1;
}
	...
	git_oid *oid, git_index *index, git_repository *repo)
{
	...
	if (git_index_has_conflicts(index)) {
		giterr_set(GITERR_INDEX,
			QT_TRANSLATE_NOOP("libgit2", "Cannot create a tree from a not fully merged index."));
	...
	const char *path)
{
	...
	if (filename_len == 0) {
		giterr_set(GITERR_TREE, QT_TRANSLATE_NOOP("libgit2", "Invalid tree path given"));
		return GIT_ENOTFOUND;
	}
	...
	if (entry == NULL) {
		giterr_set(GITERR_TREE,
			QT_TRANSLATE_NOOP("libgit2", "The path '%s' does not exist in the given tree"), path);
		return GIT_ENOTFOUND;
	}
	...
	git_vector_foreach(&tree->entries, i, entry) {
		if (preorder) {
			...
			if (error < 0) { /* negative value stops iteration */
				giterr_set_after_callback_function(error, QT_TRANSLATE_NOOP("libgit2", "git_tree_walk"));
				break;
			}
	...
	void *payload)
{
	...
	if (mode != GIT_TREEWALK_POST && mode != GIT_TREEWALK_PRE) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Invalid walking mode for tree walk"));
		return -1;
	}
