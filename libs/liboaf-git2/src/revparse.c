...
static int build_regex(regex_t *regex, const char *pattern)
{
	...
	if (*pattern == '\0') {
		giterr_set(GITERR_REGEX, QT_TRANSLATE_NOOP("libgit2", "Empty pattern"));
	...
	const char *spec)
{
	...
	giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Revspec '%s' not found."), spec);
	return GIT_ENOTFOUND;
}
...
static int retrieve_oid_from_reflog(git_oid *oid, git_reference *ref, size_t identifier)
{
	...
	if (search_by_pos) {
		if (numentries < identifier + 1) {
			...
			strcpy (message, QT_TRANSLATE_NOOP("libgit2", "Reflog for '%s' has only %"));
			strcat (message, PRIuZ);
			strcat (message, QT_TRANSLATE_NOOP("libgit2", " entries, asked for %"));
			strcat (message, PRIuZ);
		...
		case ':':
		{
			...
			if (any_left_hand_identifier(base_rev, reference, identifier_len)) {
				...
				if (*git_buf_cstr(&buf) == '/') {
					 ...
					 */
					giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unimplemented"));
					error = GIT_ERROR;
					goto cleanup;
				}
			}
	...
	if (error) {
		if (error == GIT_EINVALIDSPEC)
			giterr_set(GITERR_INVALID,
				QT_TRANSLATE_NOOP("libgit2", "Failed to parse revision specifier - Invalid pattern '%s'"), spec);
