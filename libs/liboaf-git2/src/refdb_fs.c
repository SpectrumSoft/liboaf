...
parse_failed:
	giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Corrupted packed references file"));
...
corrupted:
	giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Corrupted loose reference file: %s"), filename);
	return -1;
}
...
static const char *loose_parse_symbolic(git_buf *file_content)
{
	...
	if (git_buf_len(file_content) < header_len + 1) {
		giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Corrupted loose reference file"));
		return NULL;
	}
...
{
	giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Reference '%s' not found"), name);
	...
	int force)
{
	...
	if (!force) {
		...
		if (exists) {
			giterr_set(GITERR_REFERENCE,
				QT_TRANSLATE_NOOP("libgit2", "Failed to write reference '%s': a reference with that name already exists."), new_ref);
			return GIT_EEXISTS;
		}
	}
	...
	for (i = 0; i < git_sortedcache_entrycount(backend->refcache); ++i) {
		...
		if (ref && !ref_is_available(old_ref, new_ref, ref->name)) {
			git_sortedcache_runlock(backend->refcache);
			giterr_set(GITERR_REFERENCE,
				QT_TRANSLATE_NOOP("libgit2", "Path to reference '%s' collides with existing one"), new_ref);
			return -1;
		}
	}
...
static int packed_remove_loose(refdb_fs_backend *backend)
{
	...
	for (i = 0; i < git_sortedcache_entrycount(backend->refcache); ++i) {
		...
		if (git_path_exists(full_path.ptr) && p_unlink(full_path.ptr) < 0) {
			...
			giterr_set(GITERR_REFERENCE,
				QT_TRANSLATE_NOOP("libgit2", "Failed to remove loose reference '%s' after packing: %s"),
...
#define seek_forward(_increase) do { \
	if (_increase >= buf_size) { \
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Ran out of data while parsing reflog")); \
		goto fail; \
	} \
	buf += _increase; \
	buf_size -= _increase; \
...
static int refdb_reflog_fs__write(git_refdb_backend *_backend, git_reflog *reflog)
{
	...
	if (!git_path_isfile(git_buf_cstr(&log_path))) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "Log file for reference '%s' doesn't exist."), reflog->ref_name);
		goto cleanup;
	}
...
static int refdb_reflog_fs__rename(git_refdb_backend *_backend, const char *old_name, const char *new_name)
{
	...
	if (p_rename(git_buf_cstr(&old_path), git_buf_cstr(&temp_path)) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to rename reflog for %s"), new_name);
		error = -1;
		goto cleanup;
	}
