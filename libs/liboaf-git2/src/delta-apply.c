	...
	size_t delta_len)
{
	...
	if ((hdr_sz(&base_sz, &delta, delta_end) < 0) || (base_sz != base_len)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Failed to apply delta. Base size does not match given data"));
		return -1;
	}
...
fail:
	git__free(out->data);
	out->data = NULL;
	giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Failed to apply delta"));
	return -1;
}
