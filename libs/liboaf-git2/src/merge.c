...
int git_merge_base_many(git_oid *out, git_repository *repo, size_t length, const git_oid input_array[])
{
	...
	if (length < 2) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "At least two commits are required to find an ancestor. Provided 'length' was %u."), length);
		return -1;
	}
	...
	if (!result) {
		giterr_set(GITERR_MERGE, QT_TRANSLATE_NOOP("libgit2", "No merge base found"));
		error = GIT_ENOTFOUND;
		goto cleanup;
	}
	...
	void *payload)
{
	...
	while ((line = git__strsep(&buffer, "\n")) != NULL) {
		if (strlen(line) != GIT_OID_HEXSZ) {
			giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unable to parse OID - invalid length"));
			error = -1;
			goto cleanup;
		}
	...
	if (*buffer) {
		giterr_set(GITERR_MERGE, QT_TRANSLATE_NOOP("libgit2", "No EOL at line %d"), line_num);
		error = -1;
		goto cleanup;
	}
...
int git_merge__indexes(git_repository *repo, git_index *index_new)
{
	...
	if ((conflicts = index_conflicts + wd_conflicts) > 0) {
		giterr_set(GITERR_MERGE, QT_TRANSLATE_NOOP("libgit2", "%d uncommitted change%s would be overwritten by merge"),
			conflicts, (conflicts != 1) ? QT_TRANSLATE_NOOP("libgit2", "s") : QT_TRANSLATE_NOOP("libgit2", ""));
	...
	const git_merge_opts *given_opts)
{
	...
	*out = NULL;
	...
	if (their_heads_len != 1) {
		giterr_set(GITERR_MERGE, QT_TRANSLATE_NOOP("libgit2", "Can only merge a single branch"));
		return -1;
	}
	...
	if ((opts.merge_flags & GIT_MERGE_FASTFORWARD_ONLY) ==
		...
		giterr_set(GITERR_MERGE, QT_TRANSLATE_NOOP("libgit2", "Not a fast-forward."));
