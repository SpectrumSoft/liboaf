...
int git_clone_into(git_repository *repo, git_remote *remote, const git_checkout_opts *co_opts, const char *branch)
{
	...
	if (!git_repository_is_empty(repo)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "the repository is not empty"));
		return -1;
	}
	...
	const git_clone_options *_options)
{
	...
	if (git_path_exists(local_path) && !git_path_is_empty_dir(local_path)) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "'%s' exists and is not an empty directory"), local_path);
		return GIT_EEXISTS;
	}
