	...
	const char *path)
{
	...
	if (git_rwlock_init(&sc->lock)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to initialize lock"));
		goto fail;
	}
...
int git_sortedcache_wlock(git_sortedcache *sc)
{
	...
	if (git_rwlock_wrlock(&sc->lock) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Unable to acquire write lock on cache"));
		return -1;
	}
	return 0;
}
...
int git_sortedcache_rlock(git_sortedcache *sc)
{
	...
	if (git_rwlock_rdlock(&sc->lock) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Unable to acquire read lock on cache"));
		return -1;
	}
	return 0;
}
...
int git_sortedcache_lockandload(git_sortedcache *sc, git_buf *buf)
{
	...
	if (!git__is_sizet(sc->stamp.size)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unable to load file larger than size_t"));
		error = -1;
		goto unlock;
	}
...
int git_sortedcache_remove(git_sortedcache *sc, size_t pos)
{
	...
	if ((item = git_vector_get(&sc->items, pos)) == NULL) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Removing item out of range"));
