	...
	if (target) {
		if (git_object_owner(target) != repo) {
			giterr_set(GITERR_OBJECT,
				QT_TRANSLATE_NOOP("libgit2", "%s_default - The given target does not belong to this repository."), ERROR_MSG);
			return -1;
		}
	...
	git_reset_t reset_type)
{
	...
	if (git_object_owner(target) != repo) {
		giterr_set(GITERR_OBJECT,
			QT_TRANSLATE_NOOP("libgit2", "%s - The given target does not belong to this repository."), ERROR_MSG);
		return -1;
	}
	...
	{
		giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "%s (soft) in the middle of a merge."), ERROR_MSG);
		error = GIT_EUNMERGED;
		goto cleanup;
	}
	...
	if (reset_type > GIT_RESET_SOFT) {
		...
		if ((error = git_repository_state_cleanup(repo)) < 0) {
			giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "%s - failed to clean up merge data"), ERROR_MSG);
			goto cleanup;
		}
	}
