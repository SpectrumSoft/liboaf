...
int git_index_write(git_index *index)
{
	...
	if ((error = git_filebuf_open(
		...
		if (error == GIT_ELOCKED)
			giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "The index is locked. This might be due to a concurrrent or crashed process"));
	...
	git_index *index, const char *path, int stage)
{
	...
	if (git_index__find(&pos, index, path, stage) < 0) {
		giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "Index does not contain %s"), path);
		return NULL;
	}
...
int git_index_remove(git_index *index, const char *path, int stage)
{
	...
	if (git_index__find(&position, index, path, stage) < 0) {
		giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "Index does not contain %s at stage %d"),
			path, stage);
...
{
	giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "Invalid data in index - %s"), message);
	return -1;
}
	...
	void *payload)
{
	...
	for (i = 0; !error && i < index->entries.length; ++i) {
		...
		case INDEX_ACTION_REMOVE:
			...
			break;
		default:
			giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unknown index action %d"), action);
			error = -1;
			break;
		}
	}
