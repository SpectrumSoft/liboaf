...
on_error:
	git_buf_free(&commit);
	giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "Failed to create commit."));
	return -1;
}
...
bad_buffer:
	giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "Failed to parse bad commit object"));
	return -1;
}
	...
	git_commit **parent, const git_commit *commit, unsigned int n)
{
	...
	if (parent_id == NULL) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Parent %u does not exist"), n);
