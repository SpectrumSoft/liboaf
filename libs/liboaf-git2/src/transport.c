...
int git_transport_dummy(git_transport **transport, git_remote *owner, void *param)
{
	...
	GIT_UNUSED(owner);
	GIT_UNUSED(param);
	giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "This transport isn't implemented. Sorry"));
	return -1;
}
...
int git_transport_new(git_transport **out, git_remote *owner, const char *url)
{
	...
	if (transport_find_fn(url, &fn, &param) < 0) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Unsupported URL protocol"));
		return -1;
	}
