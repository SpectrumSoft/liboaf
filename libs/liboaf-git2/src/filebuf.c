...
static int verify_last_error(git_filebuf *file)
{
	...
	case BUFERR_WRITE:
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write out file"));
		return -1;
	...
	case BUFERR_ZLIB:
		giterr_set(GITERR_ZLIB,
			QT_TRANSLATE_NOOP("libgit2", "Buffer error when writing out ZLib data"));
		return -1;
...
static int lock_file(git_filebuf *file, int flags, mode_t mode)
{
	if (git_path_exists(file->path_lock) == true) {
		if (flags & GIT_FILEBUF_FORCE)
			...
			giterr_set(GITERR_OS,
				QT_TRANSLATE_NOOP("libgit2", "Failed to lock file '%s' for writing"), file->path_lock);
			return GIT_ELOCKED;
		}
	}
	...
	if ((flags & GIT_FILEBUF_APPEND) && git_path_exists(file->path_original) == true) {
		...
		if (source < 0) {
			giterr_set(GITERR_OS,
				QT_TRANSLATE_NOOP("libgit2", "Failed to open file '%s' for reading"),
				file->path_original);
			return -1;
		}
		...
		if (read_bytes < 0) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read file '%s'"), file->path_original);
			return -1;
		}
	}
...
int git_filebuf_open(git_filebuf *file, const char *path, int flags, mode_t mode)
{
	...
	if (compression != 0) {
		...
		if (deflateInit(&file->zs, compression) != Z_OK) {
			giterr_set(GITERR_ZLIB, QT_TRANSLATE_NOOP("libgit2", "Failed to initialize zlib"));
			goto cleanup;
		}
...
int git_filebuf_commit(git_filebuf *file)
{
	...
	if (p_close(file->fd) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to close file at '%s'"), file->path_lock);
		goto on_error;
	}
	...
	if (p_rename(file->path_lock, file->path_original) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to rename lockfile to '%s'"), file->path_original);
		goto on_error;
	}
...
int git_filebuf_stats(time_t *mtime, size_t *size, git_filebuf *file)
{
	...
	if (res < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not get stat info for '%s'"),
			file->path_original);
		return res;
	}
