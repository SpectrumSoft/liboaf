	...
	git_pathspec *ps)
{
	...
	if ((flags & GIT_PATHSPEC_NO_MATCH_ERROR) != 0 && !found_files) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "No matching files were found"));
		error = GIT_ENOTFOUND;
	}
	...
	git_pathspec *ps)
{
	...
	if ((flags & GIT_PATHSPEC_NO_MATCH_ERROR) != 0 && !found_deltas) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "No matching deltas were found"));
		error = GIT_ENOTFOUND;
	}
