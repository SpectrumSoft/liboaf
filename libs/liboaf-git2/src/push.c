...
static int check_rref(char *ref)
{
	if (git__prefixcmp(ref, "refs/")) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Not a valid reference '%s'"), ref);
		return -1;
	}
...
static int check_lref(git_push *push, char *ref)
{
	...
	if (error == GIT_ENOTFOUND)
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "src refspec '%s' does not match any existing object"), ref);
	else
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Not a valid reference '%s'"), ref);
	return -1;
}
	...
	git_vector_foreach(&push->specs, i, spec) {
		...
		if (!spec->force) {
			...
			if (!git_odb_exists(push->repo->_odb, &spec->roid)) {
				giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Cannot push missing reference"));
			...
			if (error == GIT_ENOTFOUND ||
				...
				giterr_set(GITERR_REFERENCE,
					QT_TRANSLATE_NOOP("libgit2", "Cannot push non-fastforwardable reference"));
				error = GIT_ENONFASTFORWARD;
				goto on_error;
			}
	...
	git_vector_foreach(&push->specs, i, spec) {
		if (spec->lref) {
			...
			if (git_reference_name_to_id(
				...
				giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "No such reference '%s'"), spec->lref);
				return -1;
			}
		}
...
static int do_push(git_push *push)
{
	...
	if (!transport->push) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Remote transport doesn't support push"));
		error = -1;
		goto on_error;
	}
