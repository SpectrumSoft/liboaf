	...
	const char *path)
{
	...
	if (error < 0 && sfi.ambiguous) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "Ambiguous path '%s' given to git_status_file"), sfi.expected);
		error = GIT_EAMBIGUOUS;
	}

	if (!error && !sfi.count) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "Attempt to get status of nonexistent file '%s'"), path);
		error = GIT_ENOTFOUND;
	}
