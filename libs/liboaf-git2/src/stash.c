...
{
	giterr_set(GITERR_STASH, QT_TRANSLATE_NOOP("libgit2", "Cannot stash changes - %s"), msg);
	return error;
}
	...
	struct stash_update_rules *data)
{
	...
	for (d = 0; !error && d < max_d; ++d) {
		...
		default:
			...
			giterr_set(
				GITERR_INVALID,
				QT_TRANSLATE_NOOP("libgit2", "Cannot update index. Unimplemented status (%d)"),
				delta->status);
			return -1;
		}
	...
	size_t index)
{
	...
	if (index > max - 1) {
		...
		strcpy (message, QT_TRANSLATE_NOOP("libgit2", "No stashed state at position %"));
		strcat (message, PRIuZ);

		giterr_set(GITERR_STASH, message, index);
		goto cleanup;
	}
