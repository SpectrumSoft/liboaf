...
static void net_set_error(const char *str)
{
	...
	if (win32_error) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "%s: %s"), str, win32_error);
...
static void net_set_error(const char *str)
{
	giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "%s: %s"), str, strerror(errno));
}
#endif
...
static int ssl_set_error(gitno_ssl *ssl, int error)
{
	...
	case SSL_ERROR_WANT_ACCEPT:
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "SSL error: connection failure\n"));
		break;
	case SSL_ERROR_WANT_X509_LOOKUP:
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "SSL error: x509 error\n"));
		break;
	case SSL_ERROR_SYSCALL:
		e = ERR_get_error();
		if (e > 0) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "SSL error: %s"),
					ERR_error_string(e, NULL));
			break;
		} else if (error < 0) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "SSL error: syscall failure"));
			break;
		}
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "SSL error: received early EOF"));
		break;
	case SSL_ERROR_SSL:
		e = ERR_get_error();
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "SSL error: %s"),
	...
	default:
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "SSL error: unknown error"));
		break;
	}
	return -1;
}
...
static int verify_server_cert(gitno_ssl *ssl, const char *host)
{
	...
	if (SSL_get_verify_result(ssl->ssl) != X509_V_OK) {
		giterr_set(GITERR_SSL, QT_TRANSLATE_NOOP("libgit2", "The SSL certificate is invalid"));
		return -1;
	}
...
cert_fail:
	OPENSSL_free(peer_cn);
	giterr_set(GITERR_SSL, QT_TRANSLATE_NOOP("libgit2", "Certificate host name check failed"));
	return -1;
}
...
static int gitno__close(GIT_SOCKET s)
{
	...
	if (0 != WSACleanup()) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Winsock cleanup failed"));
		return -1;
	}
...
int gitno_connect(gitno_socket *s_out, const char *host, const char *port, int flags)
{
	...
	if (WSAStartup(MAKEWORD(2,2), &wsd) != 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Winsock init failed"));
		return -1;
	}
	...
	if ((ret = p_getaddrinfo(host, port, &hints, &info)) < 0) {
		giterr_set(GITERR_NET,
			QT_TRANSLATE_NOOP("libgit2", "Failed to resolve address for %s: %s"), host, p_gai_strerror(ret));
		return -1;
	}
	...
	if (s == INVALID_SOCKET && p == NULL) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to connect to %s"), host);
		p_freeaddrinfo(info);
		return -1;
	}
	...
	if (flags & GITNO_CONNECT_SSL) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "SSL is not supported by this copy of libgit2."));
		return -1;
	}
#endif
		...
		const char *service_suffix)
{
	...
	if (!git__prefixcmp(url, prefix_http)) {
		...
		if (data->use_ssl) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Redirect from HTTPS to HTTP is not allowed"));
			goto cleanup;
		}
	...
	if (!default_port) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Unrecognized URL prefix"));
		goto cleanup;
	}
	...
	if (!error) {
		...
		if (original_host && url[0] != '/' && strcmp(original_host, data->host)) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Cross host redirect not allowed"));
			error = -1;
		}
	}
		...
		const char *default_port)
{
	...
	if (http_parser_parse_url(url, strlen(url), false, &u)) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Malformed URL '%s'"), url);
