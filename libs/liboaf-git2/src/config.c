	...
	int force)
{
	...
	if (res < 0 && errno != ENOENT) {
		giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Error stat'ing config file '%s'"), path);
		return -1;
	}
	...
	git_config_level_t level)
{
	...
	if (pos == -1) {
		giterr_set(GITERR_CONFIG,
			QT_TRANSLATE_NOOP("libgit2", "No config file exists for the given level '%i'"), (int)level);
...
static int duplicate_level(void **old_raw, void *new_raw)
{
	...
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "A file with the same level (%i) has already been added to the config"), (int)(*old)->level);
	return GIT_EEXISTS;
}
...
{
	giterr_set(GITERR_CONFIG,
		QT_TRANSLATE_NOOP("libgit2", "Cannot set value for '%s' when no config files exist"), name);
	return GIT_ENOTFOUND;
}
...
int git_config_set_string(git_config *cfg, const char *name, const char *value)
{
	...
	if (!value) {
		giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "The value to set cannot be NULL"));
		return -1;
	}
...
{
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Config value '%s' was not found"), name);
	return GIT_ENOTFOUND;
}
...
fail_parse:
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Failed to map '%s'"), value);
	return -1;
}
...
int git_config_parse_bool(int *out, const char *value)
{
	...
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Failed to parse '%s' as a boolean value"), value);
	return -1;
}
...
fail_parse:
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Failed to parse '%s' as an integer"), value);
	return -1;
}
...
fail_parse:
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Failed to parse '%s' as a 32-bit integer"), value);
	return -1;
}
...
invalid:
	git__free(name);
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Invalid config item name '%s'"), in);
	...
	const char *new_section_name)
{
	...
	{
		giterr_set(
			GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Invalid config section '%s'"), new_section_name);
		goto cleanup;
	}
