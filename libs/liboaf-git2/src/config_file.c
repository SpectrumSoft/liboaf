...
{
	giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Failed to parse config file: %s (in %s:%d, column %d)"),
...
static int config_set(git_config_backend *cfg, const char *name, const char *value)
{
	...
	if (git_strmap_valid_index(b->values, pos)) {
		...
		if (existing->next != NULL) {
			giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Multivar incompatible with simple set"));
			return -1;
		}
...
static int config_delete(git_config_backend *cfg, const char *name)
{
	...
	if (!git_strmap_valid_index(b->values, pos)) {
		giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Could not find key '%s' to delete"), name);
		return GIT_ENOTFOUND;
	}
	...
	if (var->next != NULL) {
		giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Cannot delete multivar with a single delete"));
		return -1;
	}
...
static int config_parse(diskfile_backend *cfg_file, struct reader *reader, git_config_level_t level, int depth)
{
	...
	if (depth >= MAX_INCLUDE_DEPTH) {
		giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Maximum config include depth reached"));
		return -1;
	}
		...
		if (preg_replaced) {
			...
			if (value == NULL) {
				giterr_set(GITERR_CONFIG,
					QT_TRANSLATE_NOOP("libgit2", "Race condition when writing a config file (a cvar has been removed)"));
				goto rewrite_fail;
			}
			...
			*out++ = *ptr;
			...
			if ((esc = strchr(escapes, *ptr)) != NULL) {
				*out++ = escaped[esc - escapes];
				...
				git__free(str);
				giterr_set(GITERR_CONFIG, QT_TRANSLATE_NOOP("libgit2", "Invalid escape at %s"), ptr);
				return NULL;
			}
		}
		ptr++;
