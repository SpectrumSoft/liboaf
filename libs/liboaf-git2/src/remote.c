...
static int ensure_remote_name_is_valid(const char *name)
{
	...
	if (!git_remote_is_valid_name(name)) {
		giterr_set(
			GITERR_CONFIG,
			QT_TRANSLATE_NOOP("libgit2", "'%s' is not a valid remote name."), name);
...
static int ensure_remote_doesnot_exist(git_repository *repo, const char *name)
{
	...
	giterr_set(
		GITERR_CONFIG,
		QT_TRANSLATE_NOOP("libgit2", "Remote '%s' already exists."), name);
...
int git_remote_save(const git_remote *remote)
{
	...
	if (!remote->name) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Can't save an in-memory remote."));
...
int git_remote_connect(git_remote *remote, git_direction direction)
{
	...
	if (url == NULL) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "Malformed remote '%s' - missing URL"), remote->name);
		return -1;
	}
...
int git_remote_set_transport(git_remote *remote, git_transport *transport)
{
	...
	if (remote->transport) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "A transport is already bound to this remote"));
		return -1;
	}
	...
	void *payload)
{
	...
	if (!remote->name) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Can't rename an in-memory remote."));
		return GIT_EINVALIDSPEC;
	}
