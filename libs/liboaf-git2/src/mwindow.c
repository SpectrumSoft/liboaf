...
void git_mwindow_free_all(git_mwindow_file *mwf)
{
	...
	if (git_mutex_lock(&git__mwindow_mutex)) {
		giterr_set(GITERR_THREAD, QT_TRANSLATE_NOOP("libgit2", "unable to lock mwindow mutex"));
		return;
	}
...
static int git_mwindow_close_lru(git_mwindow_file *mwf)
{
	...
	if (!lru_w) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to close memory window. Couldn't find LRU"));
		return -1;
	}
