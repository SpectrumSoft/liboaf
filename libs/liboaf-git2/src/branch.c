	...
	*branch_reference_out = NULL;
	...
	if ((error = git_reference_lookup(&branch, repo, ref_name.ptr)) < 0) {
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot locate %s branch '%s'."), is_remote ? QT_TRANSLATE_NOOP("libgit2", "remote-tracking") : QT_TRANSLATE_NOOP("libgit2", "local"), branch_name);
		goto cleanup;
	}
...
static int not_a_local_branch(const char *reference_name)
{
	giterr_set(
		GITERR_INVALID,
		QT_TRANSLATE_NOOP("libgit2", "Reference '%s' is not a local branch."), reference_name);
	return -1;
}
...
int git_branch_delete(git_reference *branch)
{
	...
	if (!git_reference_is_branch(branch) && !git_reference_is_remote(branch)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Reference '%s' is not a valid branch."),
	...
	if (is_head) {
		giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Cannot delete branch '%s' as it is the current HEAD of the repository."), git_reference_name(branch));
		return -1;
	}
...
int git_branch_name(const char **out, git_reference *ref)
{
	...
	if (git_reference_is_branch(ref)) {
		...
		giterr_set(GITERR_INVALID,
				QT_TRANSLATE_NOOP("libgit2", "Reference '%s' is neither a local nor a remote branch."), ref->name);
		return -1;
	}
	*out = branch_name;
	return 0;
	...
	const char *canonical_branch_name)
{
	...
	if (!*remote_name || !*merge_name) {
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "branch '%s' does not have an upstream"), canonical_branch_name);
		error = GIT_ENOTFOUND;
		goto cleanup;
	}
...
static int remote_name(git_buf *buf, git_repository *repo, const char *canonical_branch_name)
{
	...
	if (!git_reference__is_remote(canonical_branch_name)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Reference '%s' is not a remote branch."),
	...
	for (i = 0; i < remote_list.count; i++) {
		...
		if (fetchspec) {
			...
			if (!remote_name) {
				...
				giterr_set(GITERR_REFERENCE,
					QT_TRANSLATE_NOOP("libgit2", "Reference '%s' is ambiguous"), canonical_branch_name);
	...
	if (remote_name) {
		...
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Could not determine remote for '%s'"), canonical_branch_name);
		error = GIT_ENOTFOUND;
	}
	...
	const char *canonical_branch_name)
{
	...
	if (tracking_branch_name_out && buf.size + 1 > buffer_size) { /* +1 for NUL byte */
		giterr_set(
			GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "Buffer too short to hold the tracked reference name."));
		error = -1;
		goto cleanup;
	}
...
int git_branch_set_upstream(git_reference *branch, const char *upstream_name)
{
	...
	if (git_branch_lookup(&upstream, repo, upstream_name, GIT_BRANCH_LOCAL) == 0)
		...
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot set upstream for branch '%s'"), shortname);
		return GIT_ENOTFOUND;
	}
