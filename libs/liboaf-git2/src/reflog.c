...
int git_reflog_append(git_reflog *reflog, const git_oid *new_oid, const git_signature *committer, const char *msg)
{
	...
	if (msg != NULL) {
		...
		if (newline) {
			if (newline[1] != '\0') {
				giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Reflog message cannot contain newline"));
				goto cleanup;
			}
...
int git_reflog_drop(git_reflog *reflog, size_t idx, int rewrite_previous_entry)
{
	...
	if (entry == NULL) {
		...
		strcpy (message, QT_TRANSLATE_NOOP("libgit2", "No reflog entry at index "));
		strcat (message, PRIuZ);

		giterr_set(GITERR_REFERENCE, message, idx);
