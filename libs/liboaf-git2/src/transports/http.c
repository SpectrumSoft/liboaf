...
static int on_headers_complete(http_parser *parser)
{
	...
	if ((parser->status_code == 301 ||
		...
		if (s->redirect_count >= 7) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Too many redirects"));
			return t->parse_error = PARSE_ERROR_GENERIC;
		}
	...
	if (parser->status_code != 200) {
		giterr_set(GITERR_NET,
			QT_TRANSLATE_NOOP("libgit2", "Unexpected HTTP status code: %d"),
			parser->status_code);
		return t->parse_error = PARSE_ERROR_GENERIC;
	}
	...
	if (!t->content_type) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "No Content-Type header in response"));
		return t->parse_error = PARSE_ERROR_GENERIC;
	}
	...
	if (strcmp(t->content_type, git_buf_cstr(&buf))) {
		git_buf_free(&buf);
		giterr_set(GITERR_NET,
			QT_TRANSLATE_NOOP("libgit2", "Invalid Content-Type: %s"),
			t->content_type);
		return t->parse_error = PARSE_ERROR_GENERIC;
	}
...
static int on_body_fill_buffer(http_parser *parser, const char *str, size_t len)
{
	...
	if (ctx->buf_size < len) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Can't fit data in the buffer"));
		return t->parse_error = PARSE_ERROR_GENERIC;
	}
	...
	*bytes_read = 0;
	...
	if (!s->sent_request) {
		...
		if (gen_request(&request, s, 0) < 0) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Failed to generate request"));
			return -1;
		}
	...
	while (!*bytes_read && !t->parse_finished) {
		...
		if (bytes_parsed != t->parse_buffer.offset) {
			giterr_set(GITERR_NET,
				QT_TRANSLATE_NOOP("libgit2", "HTTP parser error: %s"),
