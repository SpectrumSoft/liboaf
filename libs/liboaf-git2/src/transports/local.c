	...
	git_pobject *obj)
{
	...
	if (git_odb_stream_write(odb_stream, (char *)git_odb_object_data(odb_obj),
		...
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Error when writing object to remote odb during local push operation. Remote odb object oid does not match local oid."));
		error = -1;
	}
	...
	git_push *push)
{
	...
	if (!remote_repo->is_bare) {
		error = GIT_EBAREREPO;
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Local push doesn't (yet) support pushing to non-bare repos."));
		goto on_error;
	}
