...
int git_smart__store_refs(transport_smart *t, int flushes)
{
	...
	do {
		...
		if (error == GIT_EBUFS) {
			...
			if (recvd == 0 && !flush) {
				giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Early EOF"));
				return -1;
			}
		...
		if (pkt->type == GIT_PKT_ERR) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Remote error: %s"), ((git_pkt_err *)pkt)->error);
			git__free(pkt);
			return -1;
		}
...
int git_smart__negotiate_fetch(git_transport *transport, git_repository *repo, const git_remote_head * const *wants, size_t count)
{
	...
	while (i < 256) {
		...
		if (i % 20 == 0) {
			if (t->cancelled.val) {
				giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "The fetch was cancelled by the user"));
				error = GIT_EUSER;
				goto on_error;
			}
			...
			if (t->caps.multi_ack || t->caps.multi_ack_detailed) {
				...
				if (pkt_type == GIT_PKT_ACK) {
					...
					giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Unexpected pkt type"));
					error = -1;
					goto on_error;
				}
			}
...
static int add_push_report_pkt(git_push *push, git_pkt *pkt)
{
	...
	switch (pkt->type) {
		...
		case GIT_PKT_FLUSH:
			return GIT_ITEROVER;
		default:
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "report-status: protocol error"));
			return -1;
	}
	...
	for (;;) {
		...
		switch (pkt->type) {
			case GIT_PKT_DATA:
				...
				break;
			case GIT_PKT_ERR:
				giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "report-status: Error reported: %s"),
