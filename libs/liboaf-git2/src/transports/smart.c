	...
	int flags)
{
	...
	if (GIT_DIRECTION_FETCH == t->direction)
		...
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Invalid direction"));
		return -1;
	}
	...
	if (t->rpc) {
		...
		if (!pkt || GIT_PKT_COMMENT != pkt->type) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Invalid response"));
			return -1;
		} else {
...
static int git_smart__ls(const git_remote_head ***out, size_t *size, git_transport *transport)
{
	...
	if (!t->have_refs) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "The transport has not yet loaded the refs"));
		return -1;
	}
...
int git_smart__negotiation_step(git_transport *transport, void *data, size_t len)
{
	...
	if (GIT_DIRECTION_FETCH != t->direction) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "This operation is only valid for fetch"));
		return -1;
	}
...
int git_smart__get_push_stream(transport_smart *t, git_smart_subtransport_stream **stream)
{
	...
	if (GIT_DIRECTION_PUSH != t->direction) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "This operation is only valid for push"));
		return -1;
	}
