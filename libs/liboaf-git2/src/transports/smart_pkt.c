...
static int ref_pkt(git_pkt **out, const char *line, size_t len)
{
	...
	if (line[GIT_OID_HEXSZ] != ' ') {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Error parsing pkt-line"));
		error = -1;
		goto error_out;
	}
...
static int32_t parse_len(const char *line)
{
	...
	for (i = 0; i < PKT_LEN_SIZE; ++i) {
		if (!isxdigit(num[i])) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Found invalid hex digit in length"));
			return -1;
		}
	}
