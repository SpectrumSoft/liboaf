...
static int winhttp_stream_connect(winhttp_stream *s)
{
	...
	if (!s->request) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to open request"));
		goto on_error;
	}
	...
	if (proxy_url) {
		...
		if (!WinHttpSetOption(s->request,
			...
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to set proxy"));
			git__free(proxy_wide);
			goto on_error;
		}
	...
	if (!WinHttpSetOption(s->request,
		...
		sizeof(disable_redirects))) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to disable redirects"));
			goto on_error;
	}
	...
	if (t->connection_data.use_ssl) {
		...
		if ((GIT_TRANSPORTFLAGS_NO_CHECK_CERT & flags) &&
			...
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to set options to ignore cert errors"));
			goto on_error;
		}
	}
	...
	int *auth_mechanism)
{
	...
	*auth_mechanism = 0;
	...
	if (!WinHttpQueryAuthSchemes(request, &supported, &first, &target)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to parse supported auth schemes")); 
		return -1;
	}
...
static int write_chunk(HINTERNET request, const char *buffer, size_t len)
{
	...
	if (!WinHttpWriteData(request,
		...
		&bytes_written)) {
		git_buf_free(&buf);
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write chunk header"));
		return -1;
	}
	...
	if (!WinHttpWriteData(request,
		buffer, (DWORD)len,
		&bytes_written)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write chunk"));
		return -1;
	}
	...
	if (!WinHttpWriteData(request,
		"\r\n", 2,
		&bytes_written)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write chunk footer"));
		return -1;
	}
...
replay:
	...
	if (++replay_count >= 7) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Too many redirects or authentication replays"));
		return -1;
	}
	...
	if (!s->received_response) {
		...
		if (s->chunked) {
			...
			if (!WinHttpWriteData(s->request,
				"0\r\n\r\n", 5,
				&bytes_written)) {
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write final chunk"));
				return -1;
			}
		}
		else if (s->post_body) {
			...
			if (INVALID_SET_FILE_POINTER == SetFilePointer(s->post_body,
				...
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to reset file pointer"));
				return -1;
			}
			...
			while (len > 0) {
				...
				if (!ReadFile(s->post_body, buffer,
					...
					!bytes_read) {
					git__free(buffer);
					giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read from temp file"));
					return -1;
				}
				...
				if (!WinHttpWriteData(s->request, buffer,
					...
					git__free(buffer);
					giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write data"));
					return -1;
				}
		...
		if (!WinHttpReceiveResponse(s->request, 0)) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to receive response"));
			return -1;
		}
		...
		if (!WinHttpQueryHeaders(s->request,
			...
			WINHTTP_NO_HEADER_INDEX)) {
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to retrieve status code"));
				return -1;
		}
		...
		if (!s->chunked &&
			...
			if (WinHttpQueryHeaders(s->request,
				...
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read Location header"));
				return -1;
			}
		...
		if (HTTP_STATUS_OK != status_code) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Request failed with status code: %d"), status_code);
			return -1;
		}
		...
		if (!WinHttpQueryHeaders(s->request,
			...
			WINHTTP_NO_HEADER_INDEX)) {
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to retrieve response content-type"));
				return -1;
		}

		if (wcscmp(expected_content_type, content_type)) {
			giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Received unexpected content-type"));
			return -1;
		}
	...
	if (!WinHttpReadData(s->request,
		...
		&dw_bytes_read))
	{
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read data"));
		return -1;
	}
	...
	size_t len)
{
	...
	if (s->sent_request) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Subtransport configured for only one write"));
		return -1;
	}
...
static int put_uuid_string(LPWSTR buffer, size_t buffer_len_cch)
{
	...
	if (RPC_S_OK != status &&
		...
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Unable to generate name for temp file"));
		return -1;
	}

	if (buffer_len_cch < UUID_LENGTH_CCH + 1) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Buffer too small for name of temp file"));
		return -1;
	}
	...
	if (FAILED(result)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Unable to generate name for temp file"));
		return -1;
	}
...
static int get_temp_file(LPWSTR buffer, DWORD buffer_len_cch)
{
	...
	if (!GetTempPathW(buffer_len_cch, buffer)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to get temp path"));
		return -1;
	}
	...
	size_t len)
{
	...
	if (!s->post_body) {
		...
		if (INVALID_HANDLE_VALUE == s->post_body) {
			s->post_body = NULL;
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to create temporary file"));
			return -1;
		}
	}

	if (!WriteFile(s->post_body, buffer, (DWORD)len, &bytes_written, NULL)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to write to temporary file"));
		return -1;
	}
...
static int winhttp_close(git_smart_subtransport *subtransport)
{
	...
	if (t->connection) {
		if (!WinHttpCloseHandle(t->connection)) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Unable to close connection"));
			ret = -1;
		}
	...
	if (t->session) {
		if (!WinHttpCloseHandle(t->session)) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Unable to close session"));
			ret = -1;
		}
