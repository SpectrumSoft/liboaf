...
static void ssh_error(LIBSSH2_SESSION *session, const char *errmsg)
{
	...
	giterr_set(GITERR_SSH, QT_TRANSLATE_NOOP("libgit2", "%s: %s"), errmsg, ssherr);
}
...
static int gen_proto(git_buf *request, const char *cmd, const char *url)
{
	...
	if (!repo) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Malformed git protocol URL"));
		return -1;
	}
	...
	const char *url)
{
	...
	if (colon == NULL || (colon < start)) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Malformed URL"));
		return -1;
	}
	...
	gitno_socket socket)
{
	...
	if (!s) {
		giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Failed to initialize SSH session"));
		return -1;
	}
	...
	git_smart_subtransport_stream **stream)
{
	...
	*stream = NULL;
	...
	if (user && pass) {
		...
		if (!t->cred) {
			giterr_set(GITERR_SSH, QT_TRANSLATE_NOOP("libgit2", "Callback failed to initialize SSH credentials"));
			goto on_error;
		}
	} else {
		giterr_set(GITERR_SSH, QT_TRANSLATE_NOOP("libgit2", "Cannot set up SSH connection without credentials"));
		goto on_error;
	}
	assert(t->cred);

	if (!user && !git_cred_has_username(t->cred)) {
		giterr_set_str(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Cannot authenticate without a username"));
		goto on_error;
	}
	...
	git_smart_subtransport_stream **stream)
{
	...
	giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Must call UPLOADPACK_LS before UPLOADPACK"));
	return -1;
}
	...
	git_smart_subtransport_stream **stream)
{
	...
	giterr_set(GITERR_NET, QT_TRANSLATE_NOOP("libgit2", "Must call RECEIVEPACK_LS before RECEIVEPACK"));
	return -1;
}
	...
	git_smart_subtransport **out, git_transport *owner)
{
	...
	*out = NULL;
	...
	giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Cannot create SSH transport. Library was built without SSH support"));
	return -1;
#endif
}
