	...
	if (diff->opts.notify_cb) {
		...
		if (error) {
			...
			if (error > 0)	/* positive value means to skip this delta */
				...
				return giterr_set_after_callback_function(error, QT_TRANSLATE_NOOP("libgit2", "git_diff"));
		}
	}
	...
	git_oid *oid)
{
	...
	if (S_ISGITLINK(mode)) {
		...
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "File size overflow (for 32-bits) on '%s'"), path);
		result = -1;
	} else {
...
int git_diff_options_init(git_diff_options *options, unsigned int version)
{
	...
	if (version != template.version) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "Invalid version %d for git_diff_options"), (int)version);
		return -1;
	}
