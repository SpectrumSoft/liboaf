...
static int read_gitfile(git_buf *path_out, const char *file_path)
{
	...
	{
		giterr_set(GITERR_REPOSITORY, QT_TRANSLATE_NOOP("libgit2", "The `.git` file at '%s' is malformed"), file_path);
		error = -1;
	}
	...
	const char *ceiling_dirs)
{
	...
	if (!git_buf_len(repo_path) && !error) {
		giterr_set(GITERR_REPOSITORY,
			QT_TRANSLATE_NOOP("libgit2", "Could not find repository from '%s'"), start_path);
		error = GIT_ENOTFOUND;
	}
	...
	const char *bare_path)
{
	...
	if (!valid_repository_path(&path)) {
		git_buf_free(&path);
		giterr_set(GITERR_REPOSITORY, QT_TRANSLATE_NOOP("libgit2", "Path is not a repository: %s"), bare_path);
	...
	const char *ceiling_dirs)
{
	...
	*repository_path = '\0';
	...
	if (size < (size_t)(path.size + 1)) {
		giterr_set(GITERR_REPOSITORY,
			QT_TRANSLATE_NOOP("libgit2", "The given buffer is too small to store the discovered path"));
		git_buf_free(&path);
		return -1;
	}
...
static int check_repositoryformatversion(git_config *config)
{
	...
	if (GIT_REPO_VERSION < version) {
		giterr_set(GITERR_REPOSITORY,
			QT_TRANSLATE_NOOP("libgit2", "Unsupported repository version %d. Only versions up to %d are supported."),
...
static int create_empty_file(const char *path, mode_t mode)
{
	...
	if ((fd = p_creat(path, mode)) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Error while creating '%s'"), path);
		return -1;
	}
	...
	const char *content)
{
	...
	if (error)
		giterr_set(GITERR_OS,
			QT_TRANSLATE_NOOP("libgit2", "Failed to initialize repository with template '%s'"), file);
	...
	const char *in_dir, const char *to_repo)
{
	...
	if (!p_stat(buf.ptr, &st) && !S_ISREG(st.st_mode)) {
		giterr_set(GITERR_REPOSITORY,
			QT_TRANSLATE_NOOP("libgit2", "Cannot overwrite gitlink file into path '%s'"), in_dir);
		error = GIT_EEXISTS;
		goto cleanup;
	}
	...
	git_repository_init_options *opts)
{
	...
	if ((opts->flags & GIT_REPOSITORY_INIT__HAS_DOTGIT) != 0) {
		if (p_hide_directory__w32(repo_dir) < 0) {
			giterr_set(GITERR_REPOSITORY,
				QT_TRANSLATE_NOOP("libgit2", "Failed to mark Git repository folder as hidden"));
			return -1;
		}
	}
#endif
	...
	git_repository_init_options *opts)
{
	...
	if (!is_bare) {
		if (opts->workdir_path) {
			...
			giterr_set(GITERR_REPOSITORY, QT_TRANSLATE_NOOP("libgit2", "Cannot pick working directory for non-bare repository that isn't a '.git' directory"));
			return -1;
		}
	...
	git_repository_init_options *opts)
{
	...
	if (valid_repository_path(&repo_path)) {
		...
		if ((opts->flags & GIT_REPOSITORY_INIT_NO_REINIT) != 0) {
			giterr_set(GITERR_REPOSITORY,
				QT_TRANSLATE_NOOP("libgit2", "Attempt to reinitialize '%s'"), given_repo);
			error = GIT_EEXISTS;
			goto cleanup;
		}
...
int git_repository_message(char *buffer, size_t len, git_repository *repo)
{
	...
	if ((error = p_stat(git_buf_cstr(&path), &st)) < 0) {
		if (errno == ENOENT)
			error = GIT_ENOTFOUND;
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not access message file"));
	}
	else if (buffer != NULL) {
	...
	const char *as_path)
{
	...
	if (!git__is_sizet(len)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "File size overflow for 32-bit systems"));
		error = -1;
		goto cleanup;
	}
