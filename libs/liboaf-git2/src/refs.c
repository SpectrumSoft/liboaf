	...
	*ref_out = NULL;
	...
	if (scan_type != GIT_REF_OID && max_nesting != 0) {
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot resolve reference (>%u levels deep)"), max_nesting);
		git_reference_free(ref);
		return -1;
	}
...
cleanup:
	if (error && !foundvalid) {
		...
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Could not use '%s' as valid reference name"), git_buf_cstr(&name));
	}
	...
	int force)
{
	...
	if (!git_odb_exists(odb, oid)) {
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Target OID for the reference doesn't exist on the repository"));
		return -1;
	}
	...
	const git_oid *id)
{
	...
	if (ref->type != GIT_REF_OID) {
		giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Cannot set OID on symbolic reference"));
		return -1;
	}
	...
	const char *target)
{
	...
	if (ref->type != GIT_REF_SYMBOLIC) {
		giterr_set(GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot set symbolic target on a direct reference"));
		return -1;
	}
	...
	int force)
{
	...
	if (should_head_be_updated &&
		...
		giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Failed to update HEAD after renaming reference"));
		return error;
	}
...
int git_reference_resolve(git_reference **ref_out, const git_reference *ref)
{
	...
	default:
		giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Invalid reference"));
		return -1;
	}
}
...
cleanup:
	if (error == GIT_EINVALIDSPEC)
		giterr_set(
			GITERR_REFERENCE,
			QT_TRANSLATE_NOOP("libgit2", "The given reference name '%s' is not valid"), name);
	...
	unsigned int flags)
{
	...
	if (git_buf_len(&buf) > buffer_size - 1) {
		giterr_set(
		GITERR_REFERENCE,
		QT_TRANSLATE_NOOP("libgit2", "The provided buffer is too short to hold the normalization of '%s'"), name);
		error = GIT_EBUFS;
		goto cleanup;
	}
	...
	int nesting)
{
	...
	if (nesting > MAX_NESTING_LEVEL) {
		giterr_set(GITERR_REFERENCE, QT_TRANSLATE_NOOP("libgit2", "Reference chain too deep (%d)"), nesting);
...
static int peel_error(int error, git_reference *ref, const char* msg)
{
	giterr_set(
		GITERR_INVALID,
		QT_TRANSLATE_NOOP("libgit2", "The reference '%s' cannot be peeled - %s"), git_reference_name(ref), msg);
	return error;
}
