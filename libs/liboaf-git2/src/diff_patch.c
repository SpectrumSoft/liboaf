	...
	git_patch *patch, git_diff_output *output)
{
	...
	return giterr_set_after_callback_function(
		...
		QT_TRANSLATE_NOOP("libgit2", "git_patch"));
}
...
static int diff_required(git_diff *diff, const char *action)
{
	if (diff)
		return 0;
	giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Must provide valid diff to %s"), action);
	return -1;
}
	...
	git_patch **patch_ptr, git_diff *diff, size_t idx)
{
	...
	if (!delta) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Index out of range for delta in diff"));
...
{
	giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Diff patch %s index out of range"), thing);
	return GIT_ENOTFOUND;
}
