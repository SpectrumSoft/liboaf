...
int git_futils_mktmp(git_buf *path_out, const char *filename, mode_t mode)
{
	...
	if ((fd = p_mkstemp(path_out->ptr)) < 0) {
		giterr_set(GITERR_OS,
			QT_TRANSLATE_NOOP("libgit2", "Failed to create temporary file '%s'"), path_out->ptr);
		return -1;
	}

	if (p_chmod(path_out->ptr, (mode & ~mask))) {
		giterr_set(GITERR_OS,
			QT_TRANSLATE_NOOP("libgit2", "Failed to set permissions on file '%s'"), path_out->ptr);
		return -1;
	}
...
int git_futils_creat_withpath(const char *path, const mode_t dirmode, const mode_t mode)
{
	...
	if (fd < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to create file '%s'"), path);
		return -1;
	}
...
int git_futils_creat_locked(const char *path, const mode_t mode)
{
	...
	if (fd < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to create locked file '%s'"), path);
...
git_off_t git_futils_filesize(git_file fd)
{
	...
	if (p_fstat(fd, &sb)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to stat file descriptor"));
		return -1;
	}
...
int git_futils_readbuffer_fd(git_buf *buf, git_file fd, size_t len)
{
	...
	if (read_size != (ssize_t)len) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read descriptor"));
		return -1;
	}
	...
	git_buf *buf, const char *path, time_t *mtime, size_t *size, int *updated)
{
	...
	if (S_ISDIR(st.st_mode)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "requested file is a directory"));
		return GIT_ENOTFOUND;
	}

	if (!git__is_sizet(st.st_size+1)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Invalid regular file stat for '%s'"), path);
		return -1;
	}
	...
	const git_buf *buf,	const char *path, int flags, mode_t mode)
{
	...
	if ((fd = p_open(path, flags, mode)) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not open '%s' for writing"), path);
		return fd;
	}

	if ((error = p_write(fd, git_buf_cstr(buf), git_buf_len(buf))) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not write to '%s'"), path);
		(void)p_close(fd);
		return error;
	}

	if ((error = p_close(fd)) < 0)
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Error while closing '%s'"), path);
...
int git_futils_mv_withpath(const char *from, const char *to, const mode_t dirmode)
{
	...
	if (p_rename(from, to) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to rename '%s' to '%s'"), from, to);
		return -1;
	}
...
int git_futils_mmap_ro_file(git_map *out, const char *path)
{
	...
	if (!git__is_sizet(len)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "File `%s` too large to mmap"), path);
		return -1;
	}
	...
	uint32_t flags)
{
	...
	if (make_path.size == 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Attempt to create empty path"));
		goto done;
	}
		...
		*tail = '\0';
		...
		if (p_mkdir(make_path.ptr, mode) < 0) {
			...
			if (p_stat(make_path.ptr, &st) < 0 || !S_ISDIR(st.st_mode)) {
				...
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to make directory '%s'"), make_path.ptr);
				goto done;
			}

			/* with exclusive create, existing dir is an error */
			if ((flags & GIT_MKDIR_EXCL) != 0) {
				giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Directory already exists '%s'"), make_path.ptr);
				error = GIT_EEXISTS;
				goto done;
			}
		}
		...
		if (((flags & GIT_MKDIR_CHMOD_PATH) != 0 ||
			...
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to set permissions on '%s'"), make_path.ptr);
			goto done;
		}
	}
	...
	if ((flags & GIT_MKDIR_VERIFY_DIR) != 0 &&
		...
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Path is not a directory '%s'"), make_path.ptr);
		error = GIT_ENOTFOUND;
	}
...
{
	if (filemsg)
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not remove directory. File '%s' %s"),
				   path, filemsg);
	else
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not remove directory '%s'"), path);
...
static int git_futils_check_selector(git_futils_dir_t which)
{
	if (which < GIT_FUTILS_DIR__MAX)
		return 0;
	giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "config directory selector out of range"));
	return -1;
}
...
int git_futils_dirs_get_str(char *out, size_t outlen, git_futils_dir_t which)
{
	...
	if (!out || path->size >= outlen) {
		giterr_set(GITERR_NOMEMORY, QT_TRANSLATE_NOOP("libgit2", "Buffer is too short for the path"));
		return GIT_EBUFS;
	}
	...
	git_buf *path, const char *name, git_futils_dir_t which, const char *label)
{
	...
	git_buf_clear(path);
	giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "The %s file '%s' doesn't exist"), label, name);
	return GIT_ENOTFOUND;
}
...
static int cp_by_fd(int ifd, int ofd, bool close_fd_when_done)
{
	...
	if (len < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Read error while copying file"));
		error = (int)len;
	}
...
static int cp_link(const char *from, const char *to, size_t link_size)
{
	...
	if (read_len != (ssize_t)link_size) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read symlink data for '%s'"), from);
		error = -1;
	}
	else {
		...
		if (p_symlink(link_data, to) < 0) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not symlink '%s' as '%s'"),
				link_data, to);
			error = -1;
		}
	}
...
static int _cp_r_callback(void *ref, git_buf *from)
{
	...
	if (exists) {
		...
		if (p_unlink(info->to.ptr) < 0) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Cannot overwrite existing file '%s'"),
				info->to.ptr);
			return GIT_EEXISTS;
		}
	}
	...
	git_futils_filestamp *stamp, const char *path)
{
	...
	if (p_stat(path, &st) < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not stat '%s'"), path);
		return GIT_ENOTFOUND;
	}
