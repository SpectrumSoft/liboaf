...
{
	giterr_set(GITERR_TAG, QT_TRANSLATE_NOOP("libgit2", "Failed to parse tag. %s"), str);
	return -1;
}
...
on_error:
	git_buf_free(&tag);
	giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "Failed to create tag annotation."));
	return -1;
}
		...
		int create_tag_annotation)
{
	...
	if (git_object_owner(target) != repo) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "The given target does not belong to this repository"));
		return -1;
	}
	...
	if (error == 0 && !allow_ref_overwrite) {
		...
		giterr_set(GITERR_TAG, QT_TRANSLATE_NOOP("libgit2", "Tag already exists"));
		return GIT_EEXISTS;
	}
...
int git_tag_create_frombuffer(git_oid *oid, git_repository *repo, const char *buffer, int allow_ref_overwrite)
{
	...
	if (tag.type != target_obj->cached.type) {
		giterr_set(GITERR_TAG, QT_TRANSLATE_NOOP("libgit2", "The type for the given target is invalid"));
		goto on_error;
	}
...
static int tags_cb(const char *ref, void *data)
{
	...
	if (!(error = git_reference_name_to_id(&oid, d->repo, ref))) {
		if ((error = d->cb(ref, &oid, d->cb_data)) != 0)
			giterr_set_after_callback_function(error, QT_TRANSLATE_NOOP("libgit2", "git_tag_foreach"));
	}
