...
int git_packbuilder_new(git_packbuilder **out, git_repository *repo)
{
	...
	*out = NULL;
	...
	{
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to initialize packbuilder mutex"));
		goto on_error;
	}
...
static int get_delta(void **out, git_odb *odb, git_pobject *po)
{
	...
	*out = NULL;
	...
	if (!delta_buf || delta_size != po->delta_size) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Delta size changed"));
		goto on_error;
	}
...
static git_pobject **compute_write_order(git_packbuilder *pb)
{
	...
	if (wo_end != pb->nr_objects) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "invalid write order"));
		return NULL;
	}
		     ...
		     unsigned long *mem_usage, int *ret)
{
	...
	*ret = 0;
	...
	if (!trg->data) {
		...
		if (sz != trg_size) {
			giterr_set(GITERR_INVALID,
				   QT_TRANSLATE_NOOP("libgit2", "Inconsistent target object length"));
			return -1;
		}
	...
	if (!src->data) {
		...
		if (sz != src_size) {
			giterr_set(GITERR_INVALID,
				   QT_TRANSLATE_NOOP("libgit2", "Inconsistent source object length"));
			return -1;
		}
...
static void *threaded_find_deltas(void *arg)
{
	...
	while (me->remaining) {
		...
		if (git_mutex_lock(&me->mutex)) {
			giterr_set(GITERR_THREAD, QT_TRANSLATE_NOOP("libgit2", "unable to lock packfile condition mutex"));
			return NULL;
		}
			  ...
			  int depth)
{
	...
	for (i = 0; i < pb->nr_threads; ++i) {
		...
		if (ret) {
			giterr_set(GITERR_THREAD, QT_TRANSLATE_NOOP("libgit2", "unable to create thread"));
			return -1;
		}
		active_threads++;
	}
