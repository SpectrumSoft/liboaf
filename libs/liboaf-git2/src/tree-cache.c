		...
		const char **buffer_in, const char *buffer_end, git_tree_cache *parent)
{
 ...
 corrupted:
	...
	giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "Corrupted TREE extension in index"));
	return -1;
}
...
int git_tree_cache_read(git_tree_cache **tree, const char *buffer, size_t buffer_size)
{
	...
	if (buffer < buffer_end) {
		giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "Corrupted TREE extension in index (unexpected trailing data)"));
		return -1;
	}
