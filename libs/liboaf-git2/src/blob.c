	...
	git_oid *oid, git_odb *odb, const char *path, git_off_t file_size)
{
	...
	if (written != file_size || read_len < 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read file into stream"));
		error = -1;
	}
	...
	git_oid *oid, git_odb *odb, const char *path, size_t link_size)
{
	...
	if (read_len != (ssize_t)link_size) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to create blob.  Can't read symlink '%s'"), path);
		git__free(link_data);
		return -1;
	}
	...
	void *payload)
{
	...
	while (1) {
		...
		if (read_bytes > BUFFER_SIZE) {
			giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "Invalid chunk size while creating blob"));
			error = GIT_EBUFS;
