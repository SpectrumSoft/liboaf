	...
	*remote_url = NULL;
	...
	if (!*line) {
		giterr_set(GITERR_FETCHHEAD,
			QT_TRANSLATE_NOOP("libgit2", "Empty line in FETCH_HEAD line %d"), line_num);
		return -1;
	}
	...
	if (strlen(oid_str) != GIT_OID_HEXSZ) {
		giterr_set(GITERR_FETCHHEAD,
			QT_TRANSLATE_NOOP("libgit2", "Invalid object ID in FETCH_HEAD line %d"), line_num);
		return -1;
	}
	...
	if (git_oid_fromstr(oid, oid_str) < 0) {
		...
		giterr_set(GITERR_FETCHHEAD, QT_TRANSLATE_NOOP("libgit2", "%s in FETCH_HEAD line %d"),
			err_msg, line_num);
		return -1;
	}
	...
	if (*line) {
		if ((is_merge_str = git__strsep(&line, "\t")) == NULL) {
			giterr_set(GITERR_FETCHHEAD,
				QT_TRANSLATE_NOOP("libgit2", "Invalid description data in FETCH_HEAD line %d"), line_num);
			return -1;
		}
		...
		if (*is_merge_str == '\0')
			...
			*is_merge = 0;
			...
			giterr_set(GITERR_FETCHHEAD,
				QT_TRANSLATE_NOOP("libgit2", "Invalid for-merge entry in FETCH_HEAD line %d"), line_num);
			return -1;
		}

		if ((desc = line) == NULL) {
			giterr_set(GITERR_FETCHHEAD,
				QT_TRANSLATE_NOOP("libgit2", "Invalid description in FETCH_HEAD line %d"), line_num);
			return -1;
		}
	...
	void *payload)
{
	...
	if (*buffer) {
		giterr_set(GITERR_FETCHHEAD, QT_TRANSLATE_NOOP("libgit2", "No EOL at line %d"), line_num+1);
		error = -1;
		goto done;
	}
