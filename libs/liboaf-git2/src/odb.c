...
int git_odb__hashfd(git_oid *out, git_file fd, size_t size, git_otype type)
{
	...
	if (!git_object_typeisloose(type)) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Invalid object type for hash"));
		return -1;
	}
	...
	if (read_len < 0 || size > 0) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Error reading file for hashing"));
		error = -1;
...
int git_odb__hashlink(git_oid *out, const char *path)
{
	...
	if (S_ISLNK(st.st_mode)) {
		...
		if (read_len != (ssize_t)size) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to read symlink data for '%s'"), path);
			git__free(link_data);
			return -1;
		}
...
static int git_odb__error_unsupported_in_backend(const char *action)
{
	giterr_set(GITERR_ODB,
		QT_TRANSLATE_NOOP("libgit2", "Cannot %s - unsupported in the loaded odb backends"), action);
	return -1;
}
...
int git_odb_get_backend(git_odb_backend **out, git_odb *odb, size_t pos)
{
	...
	strcpy (message, QT_TRANSLATE_NOOP("libgit2", "No ODB backend loaded at index %"));
	strcat (message, PRIuZ);

	giterr_set(GITERR_ODB, message, pos);
	...
	bool as_alternates, int alternate_depth)
{
	...
	if (p_stat(objects_dir, &st) < 0) {
		...
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Failed to load object database in '%s'"), objects_dir);
		return -1;
	}
	...
	const char *action)
{
	...
	strcpy (message, QT_TRANSLATE_NOOP("libgit2", "Cannot %s - Invalid length. %"));
	strcat (message, PRIuZ);
	strcat (message, QT_TRANSLATE_NOOP("libgit2", " was expected. The total size of the received chunks amounts to %"));
	strcat (message, PRIuZ".");
...
int git_odb__error_notfound(const char *message, const git_oid *oid)
{
	if (oid != NULL) {
		...
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Object not found - %s (%s)"), message, oid_str);
	} else
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Object not found - %s"), message);
...
{
	giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Ambiguous SHA1 prefix - %s"), message);
