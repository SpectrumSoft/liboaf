	...
	const git_index_entry *wditem)
{
...

	{
		...
		return giterr_set_after_callback_function(
			error, QT_TRANSLATE_NOOP("libgit2", "git_checkout notification"));
	}
}
	...
	const git_index_name_entry *name_entry)
{
	...
	*theirs_out = NULL;
	...
	if (!name_entry->ancestor) {
		giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "A NAME entry exists without an ancestor"));
		error = -1;
		goto done;
	}

	if (!name_entry->ours && !name_entry->theirs) {
		giterr_set(GITERR_INDEX, QT_TRANSLATE_NOOP("libgit2", "A NAME entry exists without an ours or theirs"));
		error = -1;
		goto done;
	}
	...
	if ((ancestor = checkout_conflicts_search_ancestor(data,
		...
		giterr_set(GITERR_INDEX,
			QT_TRANSLATE_NOOP("libgit2", "A NAME entry referenced ancestor entry '%s' which does not exist in the main index"),
			name_entry->ancestor);
		error = -1;
		goto done;
	}
	...
	if (name_entry->ours) {
		if (strcmp(name_entry->ancestor, name_entry->ours) == 0)
			...
			ours->ours == NULL) {
			giterr_set(GITERR_INDEX,
				QT_TRANSLATE_NOOP("libgit2", "A NAME entry referenced our entry '%s' which does not exist in the main index"),
				name_entry->ours);
			error = -1;
			goto done;
		}
	...
	if (name_entry->theirs) {
		if (strcmp(name_entry->ancestor, name_entry->theirs) == 0)
			...
			theirs->theirs == NULL) {
			giterr_set(GITERR_INDEX,
				QT_TRANSLATE_NOOP("libgit2", "A NAME entry referenced their entry '%s' which does not exist in the main index"),
				name_entry->theirs);
			error = -1;
			goto done;
		}
	...
	git_vector_foreach(&data->conflicts, i, conflict) {
		...
		if ((error = git_index_find(&j, data->index, path)) < 0) {
			if (error == GIT_ENOTFOUND)
				giterr_set(GITERR_INDEX,
					QT_TRANSLATE_NOOP("libgit2", "Index inconsistency, could not find entry for expected conflict '%s'"), path);
		...
		for (; j < len; j++) {
			if ((entry = git_index_get_byindex(data->index, j)) == NULL) {
				giterr_set(GITERR_INDEX,
					QT_TRANSLATE_NOOP("libgit2", "Index inconsistency, truncated index while loading expected conflict '%s'"), path);
				error = -1;
				goto done;
			}
	...
	git_iterator *workdir)
{
	...
	*actions_ptr = actions = git__calloc(
	...
	{
		giterr_set(GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "%d %s checkout"),
			(int)counts[CHECKOUT_ACTION__CONFLICT],
			counts[CHECKOUT_ACTION__CONFLICT] == 1 ?
			QT_TRANSLATE_NOOP("libgit2", "conflict prevents") : QT_TRANSLATE_NOOP("libgit2", "conflicts prevent"));
	...
	mode_t file_mode)
{
	...
	if (st != NULL && (error = p_stat(path, st)) < 0)
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Error statting '%s'"), path);

	else if (GIT_PERMS_IS_EXEC(file_mode) &&
			(error = p_chmod(path, file_mode)) < 0)
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to set permissions on '%s'"), path);
	...
	int can_symlink)
{
	...
	if (can_symlink) {
		if ((error = p_symlink(git_buf_cstr(&linktarget), path)) < 0)
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Could not create symlink %s\n"), path);
	} else {
	...
	if (!error) {
		if ((error = p_lstat(path, st)) < 0)
			giterr_set(GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "Could not stat symlink %s"), path);
	...
	const git_diff_file *file)
{
	...
	if (p_stat(git_buf_cstr(&data->path), &st) < 0) {
		giterr_set(
			GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "Could not stat submodule %s\n"), file->path);
...
static int checkout_safe_for_update_only(const char *path, mode_t expected_mode)
{
	...
	if (p_lstat(path, &st) < 0) {
		...
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to stat file '%s'"), path);
		return -1;
	}
...
static int checkout_path_suffixed(git_buf *path, const char *suffix)
{
	...
	if (i == INT_MAX) {
		...
		giterr_set(GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "Could not write '%s': working directory file exists"), path);
		return GIT_EEXISTS;
	}
	...
	checkout_conflictdata *conflict)
{
	...
	if (result.path == NULL || result.mode == 0) {
		giterr_set(GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "Could not merge contents of file"));
		error = GIT_EMERGECONFLICT;
		goto done;
	}
	...
	const git_checkout_opts *proposed)
{
	...
	if (!repo) {
		giterr_set(GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "Cannot checkout nothing"));
		return -1;
	}
		...
		if ((data->index = git_iterator_get_index(target)) != NULL) {
			...
			if ((data->opts.checkout_strategy & GIT_CHECKOUT_FORCE) == 0 &&
				...
				giterr_set(GITERR_CHECKOUT,
					QT_TRANSLATE_NOOP("libgit2", "unresolved conflicts exist in the index"));
				goto cleanup;
			}
	...
	const git_checkout_opts *opts)
{
	...
	if (!index && !repo) {
		giterr_set(GITERR_CHECKOUT,
			QT_TRANSLATE_NOOP("libgit2", "Must provide either repository or index to checkout"));
		return -1;
	}
	if (index && repo && git_index_owner(index) != repo) {
		giterr_set(GITERR_CHECKOUT,
			QT_TRANSLATE_NOOP("libgit2", "Index to checkout does not match repository"));
		return -1;
	}
	...
	const git_checkout_opts *opts)
{
	...
	if (!treeish && !repo) {
		giterr_set(GITERR_CHECKOUT,
			QT_TRANSLATE_NOOP("libgit2", "Must provide either repository or tree to checkout"));
		return -1;
	}
	if (treeish && repo && git_object_owner(treeish) != repo) {
		giterr_set(GITERR_CHECKOUT,
			QT_TRANSLATE_NOOP("libgit2", "Object to checkout does not match repository"));
		return -1;
	}
	...
	if (treeish) {
		if (git_object_peel((git_object **)&tree, treeish, GIT_OBJ_TREE) < 0) {
			giterr_set(
				GITERR_CHECKOUT, QT_TRANSLATE_NOOP("libgit2", "Provided object cannot be peeled to a tree"));
			return -1;
		}
	}
	else {
		if ((error = checkout_lookup_head_tree(&tree, repo)) < 0) {
			if (error != GIT_EUNBORNBRANCH)
				giterr_set(
					GITERR_CHECKOUT,
					QT_TRANSLATE_NOOP("libgit2", "HEAD could not be peeled to a tree and no treeish given"));
			return error;
		}
	}
