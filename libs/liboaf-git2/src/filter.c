	...
	const char *name, git_filter *filter, int priority)
{
	...
	if (!filter_registry_find(NULL, name)) {
		giterr_set(
			GITERR_FILTER, QT_TRANSLATE_NOOP("libgit2", "Attempt to reregister existing filter '%s'"), name);
		return GIT_EEXISTS;
	}
...
int git_filter_unregister(const char *name)
{
	...
	if (!strcmp(GIT_FILTER_CRLF, name) || !strcmp(GIT_FILTER_IDENT, name)) {
		giterr_set(GITERR_FILTER, QT_TRANSLATE_NOOP("libgit2", "Cannot unregister filter '%s'"), name);
		return -1;
	}

	if ((fdef = filter_registry_lookup(&pos, name)) == NULL) {
		giterr_set(GITERR_FILTER, QT_TRANSLATE_NOOP("libgit2", "Cannot find filter '%s' to unregister"), name);
	...
	git_filter_list *fl, git_filter *filter, void *payload)
{
	...
	if (git_vector_search2(
		...
		giterr_set(GITERR_FILTER, QT_TRANSLATE_NOOP("libgit2", "Cannot use an unregistered filter"));
		return -1;
	}
	...
	git_blob *blob)
{
	...
	if (!git__is_sizet(rawsize)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Blob is too large to filter"));
		return -1;
	}
