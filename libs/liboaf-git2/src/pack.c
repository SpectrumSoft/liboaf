...
{
	giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Invalid pack file - %s"), message);
	return -1;
}
...
static int cache_init(git_pack_cache *cache)
{
	...
	if (git_mutex_init(&cache->lock)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to initialize pack cache mutex"));
...
static int cache_add(git_pack_cache *cache, git_rawobj *base, git_off_t offset)
{
	...
	if (entry) {
		if (git_mutex_lock(&cache->lock) < 0) {
			giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "failed to lock cache"));
			return -1;
		}
...
static int pack_index_check(const char *path, struct git_pack_file *p)
{
	...
	if (p_fstat(fd, &st) < 0) {
		p_close(fd);
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Unable to stat pack index '%s'"), path);
		return -1;
	}
	...
	{
		p_close(fd);
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Invalid pack index '%s'"), path);
		return -1;
	}
...
int git_packfile_stream_open(git_packfile_stream *obj, struct git_pack_file *p, git_off_t curpos)
{
	...
	if (st != Z_OK) {
		git__free(obj);
		giterr_set(GITERR_ZLIB, QT_TRANSLATE_NOOP("libgit2", "Failed to inflate packfile"));
		return -1;
	}
...
cleanup:
	giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Invalid packfile '%s'"), p->pack_name);
...
int git_packfile_alloc(struct git_pack_file **pack_out, const char *path)
{
	...
	*pack_out = NULL;
	...
	if (git_mutex_init(&p->lock)) {
		giterr_set(GITERR_OS, QT_TRANSLATE_NOOP("libgit2", "Failed to initialize packfile mutex"));
		git__free(p);
		return -1;
	}
