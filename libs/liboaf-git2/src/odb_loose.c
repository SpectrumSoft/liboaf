...
static int finish_inflate(z_stream *s)
{
	...
	if ((status != Z_STREAM_END) || (s->avail_in != 0)) {
		giterr_set(GITERR_ZLIB, QT_TRANSLATE_NOOP("libgit2", "Failed to finish ZLib inflation. Stream aborted prematurely"));
		return -1;
	}
...
static int inflate_buffer(void *in, size_t inlen, void *out, size_t outlen)
{
	...
	if (inflateInit(&zs) < Z_OK) {
		giterr_set(GITERR_ZLIB, QT_TRANSLATE_NOOP("libgit2", "Failed to inflate buffer"));
		return -1;
	}
	...
	if (status != Z_STREAM_END /* || zs.avail_in != 0 */ ||
		zs.total_out != outlen)
	{
		giterr_set(GITERR_ZLIB, QT_TRANSLATE_NOOP("libgit2", "Failed to inflate buffer. Stream aborted prematurely"));
		return -1;
	}
...
static int inflate_packlike_loose_disk_obj(git_rawobj *out, git_buf *obj)
{
	...
	if ((used = get_binary_object_header(&hdr, obj)) == 0 ||
		...
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Failed to inflate loose object."));
		return -1;
	}
...
static int inflate_disk_obj(git_rawobj *out, git_buf *obj)
{
	...
	if (start_inflate(&zs, obj, head, sizeof(head)) < Z_OK ||
		...
		!git_object_typeisloose(hdr.type))
	{
		giterr_set(GITERR_ODB, QT_TRANSLATE_NOOP("libgit2", "Failed to inflate disk object."));
		return -1;
	}
...
static int read_header_loose(git_rawobj *out, git_buf *loc)
{
	...
	{
		giterr_set(GITERR_ZLIB, QT_TRANSLATE_NOOP("libgit2", "Failed to read loose object header"));
		error = -1;
	} else {
...
static int foreach_object_dir_cb(void *_state, git_buf *path)
{
	...
	return giterr_set_after_callback_function(
		state->cb(&oid, state->data), QT_TRANSLATE_NOOP("libgit2", "git_odb_foreach"));
}
