	...
	*object_out = NULL;
	...
	if (type != GIT_OBJ_ANY && type != odb_obj->cached.type) {
		giterr_set(GITERR_INVALID,
			QT_TRANSLATE_NOOP("libgit2", "The requested type does not match the type in the ODB"));
		return GIT_ENOTFOUND;
	}

	if ((object_size = git_object__size(odb_obj->cached.type)) == 0) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "The requested type is invalid"));
		return GIT_ENOTFOUND;
	}
	...
	git_otype type)
{
	...
	if (len < GIT_OID_MINPREFIXLEN) {
		giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "Ambiguous lookup - OID prefix is too short"));
		...
		if (cached != NULL) {
			if (cached->flags == GIT_CACHE_STORE_PARSED) {
				...
				if (type != GIT_OBJ_ANY && type != object->cached.type) {
					...
					giterr_set(GITERR_INVALID,
						QT_TRANSLATE_NOOP("libgit2", "The requested type does not match the type in ODB"));
					return GIT_ENOTFOUND;
				}
...
static int peel_error(int error, const git_oid *oid, git_otype type)
{
	...
	giterr_set(GITERR_OBJECT, QT_TRANSLATE_NOOP("libgit2", "The git_object of id '%s' can not be successfully peeled into a %s (git_otype=%i)."), hex_oid, type_name, type);
		...
		git_otype type)
{
	...
	if (type != GIT_OBJ_ANY && git_tree_entry_type(entry) != type)
	{
		giterr_set(GITERR_OBJECT,
				QT_TRANSLATE_NOOP("libgit2", "object at path '%s' is not of the asked-for type %d"),
				path, type);
