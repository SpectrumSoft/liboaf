...
{
	giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unable to parse OID - %s"), msg);
	return -1;
}
...
int git_oid_shorten_add(git_oid_shorten *os, const char *text_oid)
{
	...
	if (os->full) {
		giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unable to shorten OID - OID set full"));
		return -1;
	}
	...
	for (i = 0; i < GIT_OID_HEXSZ; ++i) {
		...
		if (c == -1) {
			giterr_set(GITERR_INVALID, QT_TRANSLATE_NOOP("libgit2", "Unable to shorten OID - invalid hex value"));
			return -1;
		}
