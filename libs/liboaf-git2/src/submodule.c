	...
	const char *name)       /* trailing slash is allowed */
{
	...
	if (!git_strmap_valid_index(repo->submodules, pos)) {
		...
		giterr_set(GITERR_SUBMODULE, (error == GIT_ENOTFOUND) ?
			QT_TRANSLATE_NOOP("libgit2", "No submodule named '%s'") :
			QT_TRANSLATE_NOOP("libgit2", "Submodule '%s' has not been added yet"), name);
	...
	int use_gitlink)
{
	...
	if (git_submodule_lookup(&sm, repo, path) < 0)
		...
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "Attempt to add a submodule that already exists"));
		return GIT_EEXISTS;
	}
	...
	if (url[0] == '.' && (url[1] == '/' || (url[1] == '.' && url[2] == '/'))) {
		...
		giterr_set(GITERR_SUBMODULE, QT_TRANSLATE_NOOP("libgit2", "Invalid format for submodule URL"));
		error = -1;
	}
	if (error)
		goto cleanup;
	...
	if (git_path_root(path) >= 0) {
		giterr_set(GITERR_SUBMODULE, QT_TRANSLATE_NOOP("libgit2", "Submodule path must be a relative path"));
		error = -1;
		goto cleanup;
	}
	...
	if ((mods = open_gitmodules(repo, true, NULL)) == NULL) {
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "Adding submodules to a bare repository is not supported (for now)"));
		return -1;
	}
...
int git_submodule_add_to_index(git_submodule *sm, int write_index)
{
	...
	if (p_stat(path.ptr, &st) < 0) {
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot add submodule without working directory"));
		error = -1;
		goto cleanup;
	}
	...
	if ((sm->flags & GIT_SUBMODULE_STATUS__WD_OID_VALID) == 0) {
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot add submodule without HEAD to index"));
		error = -1;
		goto cleanup;
	}
...
int git_submodule_init(git_submodule *submodule, int overwrite)
{
	...
	if (!submodule->url) {
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "No URL configured for submodule '%s'"), submodule->name);
		return -1;
	}
...
static git_submodule *submodule_alloc(git_repository *repo, const char *name)
{
	...
	if (!name || !(namelen = strlen(name))) {
		giterr_set(GITERR_SUBMODULE, QT_TRANSLATE_NOOP("libgit2", "Invalid submodule name"));
		return NULL;
	}
...
static int submodule_config_error(const char *property, const char *value)
{
	giterr_set(GITERR_INVALID,
		QT_TRANSLATE_NOOP("libgit2", "Invalid value for submodule '%s' property: '%s'"), property, value);
	return -1;
}
...
static int lookup_head_remote(git_buf *url, git_repository *repo)
{
	...
	if (git_reference_lookup(&head, repo, GIT_HEAD_FILE) < 0) {
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot resolve relative URL when HEAD cannot be resolved"));
		error = GIT_ENOTFOUND;
		goto cleanup;
	}
	...
	if (git_reference_type(head) != GIT_REF_SYMBOLIC) {
		giterr_set(GITERR_SUBMODULE,
			QT_TRANSLATE_NOOP("libgit2", "Cannot resolve relative URL when HEAD is not symbolic"));
		error = GIT_ENOTFOUND;
		goto cleanup;
	}
