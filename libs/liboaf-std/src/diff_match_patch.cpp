 ...
 * @param operation One of _INSERT, _DELETE or _EQUAL
...
QList<Diff> diff_match_patch::diff_main(const QString &text1,
  ...
  if (text1 == text2) {
	if (!text1.isEmpty()) {
	  diffs.append(Diff(_EQUAL, text1));
	}
	return diffs;
  }
  ...
  if (!commonprefix.isEmpty()) {
	diffs.prepend(Diff(_EQUAL, commonprefix));
  }
  if (!commonsuffix.isEmpty()) {
	diffs.append(Diff(_EQUAL, commonsuffix));
  }
...
QList<Diff> diff_match_patch::diff_compute(QString text1, QString text2,
  ...
  if (text1.isEmpty()) {
	...
	diffs.append(Diff(_INSERT, text2));
	return diffs;
  }

  if (text2.isEmpty()) {
	// Just delete some text (speedup).
	diffs.append(Diff(_DELETE, text1));
	return diffs;
  }

  {
	...
	const Operation op = (text1.length() > text2.length()) ? _DELETE : _INSERT;
	  diffs.append(Diff(op, longtext.left(i)));
	  diffs.append(Diff(_EQUAL, shorttext));
  ...
  if (hm.count() > 0) {
	...
	// Merge the results.
	diffs = diffs_a;
	diffs.append(Diff(_EQUAL, mid_common));
	diffs += diffs_b;
	return diffs;
  }
...
void diff_match_patch::diff_cleanupSemantic(QList<Diff> &diffs) {
  ...
  while (thisDiff != NULL) {
	if (thisDiff->operation == _EQUAL) {
	  ...
	  if (!lastequality.isNull()
		...
		pointer.setValue(Diff(_DELETE, lastequality));
		// Insert a corresponding an insert.
		pointer.insert(Diff(_INSERT, lastequality));
  ...
  while (thisDiff != NULL) {
	if (prevDiff->operation == _DELETE &&
		thisDiff->operation == _INSERT) {
	  ...
	  if (overlap_length1 >= overlap_length2) {
		if (overlap_length1 >= deletion.length() / 2.0 ||
		  ...
		  pointer.previous();
		  pointer.insert(Diff(_EQUAL, insertion.left(overlap_length1)));
		  prevDiff->text =
		...
		if (overlap_length2 >= deletion.length() / 2.0 ||
		  ...
		  pointer.previous();
		  pointer.insert(Diff(_EQUAL, deletion.left(overlap_length2)));
		  prevDiff->operation = _INSERT;
		  prevDiff->text =
			  insertion.left(insertion.length() - overlap_length2);
		  thisDiff->operation = _DELETE;
...
void diff_match_patch::diff_cleanupSemanticLossless(QList<Diff> &diffs) {
  ...
  while (nextDiff != NULL) {
	if (prevDiff->operation == _EQUAL &&
	  nextDiff->operation == _EQUAL) {
...
void diff_match_patch::diff_cleanupMerge(QList<Diff> &diffs) {
  diffs.append(Diff(_EQUAL, ""));  // Add a dummy entry at the end.
  QMutableListIterator<Diff> pointer(diffs);
  int count_delete = 0;
  int count_insert = 0;
  QString text_delete = "";
	  ...
	  case _EQUAL:
		if (count_delete + count_insert > 1) {
		  ...
		  if (both_types) {
			...
			if (commonlength != 0) {
			  if (pointer.hasPrevious()) {
				...
				pointer.insert(Diff(_EQUAL, text_insert.left(commonlength)));
			  }
		  ...
		  if (!text_delete.isEmpty()) {
			pointer.insert(Diff(_DELETE, text_delete));
		  }
		  if (!text_insert.isEmpty()) {
			pointer.insert(Diff(_INSERT, text_insert));
		  }
...
int diff_match_patch::diff_xIndex(const QList<Diff> &diffs, int loc) {
  ...
  Diff lastDiff;
  foreach(Diff aDiff, diffs) {
	if (aDiff.operation != _INSERT) {
	  // Equality or deletion.
	  chars1 += aDiff.text.length();
	}
	if (aDiff.operation != _DELETE) {
...
QString diff_match_patch::diff_prettyHtml(const QList<Diff> &diffs) {
  ...
  foreach(Diff aDiff, diffs) {
	...
	text.replace("&", "&amp;").replace("<", "&lt;")
		.replace(">", "&gt;")
		// NOTE: Изначально была замена на "&para;<br>" - но в QWebView она отображается некорректно
		.replace("\n", "<br>");
	switch (aDiff.operation) {
	  case _INSERT:
  ...
  foreach(QString token, tokens) {
	...
	QString param = safeMid(token, 1);
	  ...
	  case '+':
		...
		diffs.append(Diff(_INSERT, param));
		break;
	  case '-':
		// Fall through.
	  case '=': {
		...
		pointer += n;
		if (token[0] == QChar('=')) {
		  diffs.append(Diff(_EQUAL, text));
		} else {
		  diffs.append(Diff(_DELETE, text));
		}
		break;
	  }
	  default:
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text) {
  ...
  if (!prefix.isEmpty()) {
	patch.diffs.prepend(Diff(_EQUAL, prefix));
  }
  // Add the suffix.
  ...
  if (!suffix.isEmpty()) {
	patch.diffs.append(Diff(_EQUAL, suffix));
  }
...
QList<Patch> diff_match_patch::patch_make(const QString &text1,
  ...
  foreach(Diff aDiff, diffs) {
	if (patch.diffs.isEmpty() && aDiff.operation != _EQUAL) {
...
QString diff_match_patch::patch_addPadding(QList<Patch> &patches) {
  ...
  if (firstPatchDiffs.empty() || firstPatchDiffs.first().operation != _EQUAL) {
	// Add nullPadding equality.
	firstPatchDiffs.prepend(Diff(_EQUAL, nullPadding));
  ...
  if (lastPatchDiffs.empty() || lastPatchDiffs.last().operation != _EQUAL) {
	// Add nullPadding equality.
	lastPatchDiffs.append(Diff(_EQUAL, nullPadding));
	...
	while (!bigpatch.diffs.isEmpty()) {
	  ...
	  while (!bigpatch.diffs.isEmpty()
		...
		if (diff_type == _INSERT) {
		  ...
		  empty = false;
		} else if (diff_type == _DELETE && patch.diffs.size() == 1
			&& patch.diffs.front().operation == _EQUAL
	  ...
	  if (!postcontext.isEmpty()) {
		...
		if (!patch.diffs.isEmpty()
			&& patch.diffs.back().operation == _EQUAL) {
		  patch.diffs.back().text += postcontext;
		} else {
		  patch.diffs.append(Diff(_EQUAL, postcontext));
		}
	  }
	  if (!empty) {
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline) {
  ...
  while (!text.isEmpty()) {
	...
	while (!text.isEmpty()) {
	  ...
	  if (sign == '-') {
		// Deletion.
		patch.diffs.append(Diff(_DELETE, line));
	  } else if (sign == '+') {
		// Insertion.
		patch.diffs.append(Diff(_INSERT, line));
	  } else if (sign == ' ') {
		// Minor equality.
		patch.diffs.append(Diff(_EQUAL, line));
	  } else if (sign == '@') {
		// Start of next patch.
		break;
	  } else {
